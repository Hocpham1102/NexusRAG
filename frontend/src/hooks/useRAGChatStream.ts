...
export function useRAGChatStream(workspaceId: string): RAGStreamResult {
  ...
  const sendMessage = useCallback(
    async (
      ...
      function syncUpdateSteps(
        updater: AgentStep[] | ((prev: AgentStep[]) => AgentStep[]),
      ): void {
        const next =
          typeof updater === "function" ? updater(localSteps) : updater;
        localSteps = next;
        setAgentSteps(next);
      }
                ...
                switch (currentEventType) {
                  ...
                  case "complete": {
                        ...
                        prev.map((s) =>
                          s.step === "analyzing"
                            ? {
                                ...s,
                                thinkingText:
                                  (s.thinkingText || "") + thinkingAccumulator,
                              }
                            : s,
                        ),
                      );
                    ...
                    syncUpdateSteps((prev) => [
                      ...
                      createStep(
                        "done",
                        `Done in ${totalMs >= 1000 ? `${(totalMs / 1000).toFixed(1)}s` : `${totalMs}ms`}`,
                        "completed",
                      ),
                    ]);
