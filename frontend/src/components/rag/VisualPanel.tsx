...
import { BookOpen, FileSearch, List, Network } from "lucide-react";
...
function SubTabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex items-center gap-1 px-2 py-1 text-[11px] font-medium rounded-md transition-colors border",
        active
          ? "bg-primary/10 text-primary border-primary/25"
          : "text-muted-foreground hover:text-foreground hover:bg-muted/50 border-border/55",
      )}
    >
      {icon}
      {label}
...
export const VisualPanel = memo(function VisualPanel({
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden min-h-0 rounded-2xl bg-gradient-to-b from-card/96 via-card/88 to-card/74">
      {/* Tab bar */}
      <div className="flex-shrink-0 flex items-center gap-1 px-3.5 py-2.5 border-b border-border/70 bg-background/30">
        <TabButton
        ...
        {highlightChunks.length > 0 && (
          <span className="ml-auto text-[10px] text-primary bg-primary/10 border border-primary/25 px-2 py-0.5 rounded-full">
