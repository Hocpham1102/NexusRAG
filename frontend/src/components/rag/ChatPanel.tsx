...
// ---------------------------------------------------------------------------
const CITATION_RE =
  /(\[(?:[a-z0-9]+|IMG-[a-z0-9]+)(?:,\s*(?:[a-z0-9]+|IMG-[a-z0-9]+))*\])/g;
      ...
      parts.forEach((part, i) => {
        ...
        tokens.forEach((token, ti) => {
          ...
          const source =
            ...
            (fallbackSources
              ? fallbackSources.find((s) => String(s.index) === token)
              : undefined);
          if (source) {
            result.push(
              <CitationLink
...
// ---------------------------------------------------------------------------
const CITATION_STRIP_RE =
  /\s*\[(?:[a-z0-9]+|IMG-[a-z0-9]+)(?:,\s*(?:[a-z0-9]+|IMG-[a-z0-9]+))*\]/g;
...
function CopyMessageActions({ content }: { content: string }) {
  const [copiedMode, setCopiedMode] = useState<"text" | "markdown" | null>(
    null,
  );
...
const MessageBubble = memo(function MessageBubble({
  ...
  const retrievalLabel =
    message.retrievalMode === "graphrag"
      ? "GraphRAG"
      : message.retrievalMode === "rag"
        ? "RAG"
        : null;
    ...
    >
      ...
      >
        ...
        {!isUser &&
          message.isStreaming &&
          !message.content &&
          !message.agentSteps?.length && <TypingIndicator status="analyzing" />}
...
export const ChatPanel = memo(function ChatPanel({
  ...
  const [queryMode, setQueryMode] = useState<"rag" | "graphrag" | "compare">(
    () => {
      const saved = localStorage.getItem("nexusrag-query-mode");
      if (saved === "rag" || saved === "graphrag" || saved === "compare") {
        return saved;
      }

      // Backward compatibility for previous separate toggles.
      if (localStorage.getItem("nexusrag-compare-modes") === "true") {
        return "compare";
      }
      return localStorage.getItem("nexusrag-rag-mode") === "graphrag"
        ? "graphrag"
        : "rag";
    },
  );
  ...
  const handleSavePrompt = useCallback(() => {
    ...
    const toSave =
      promptDraft.trim() === DEFAULT_SYSTEM_PROMPT ? "" : promptDraft;
  ...
  useEffect(() => {
    ...
    setMessages((prev) => {
      ...
      const newSources =
        stream.pendingSources.length > 0 ? stream.pendingSources : m.sources;
      const newImages =
        stream.pendingImages.length > 0 ? stream.pendingImages : m.imageRefs;
      const newThinking = stream.thinkingText || m.thinking;
      const newSteps =
        stream.agentSteps.length > 0 ? stream.agentSteps : m.agentSteps;
  ...
  const handleSend = useCallback(
    async (text?: string) => {
      ...
      const modeOrder: ("rag" | "graphrag")[] =
        queryMode === "compare" ? ["rag", "graphrag"] : [queryMode];
      ...
      const placeholders: ChatMessage[] = modeOrder.map((mode, index) => ({
        id: generateId(),
        role: "assistant",
        content:
          index === 0 ? "" : "Waiting to run comparison for this mode...",
        timestamp: new Date().toISOString(),
        isStreaming: index === 0,
        retrievalMode: mode,
      }));
      ...
      for (const [index, mode] of modeOrder.entries()) {
        const assistantId = placeholders[index].id;
        ...
        // Cancelled — keep partial content and stop pending compare runs
        setMessages((prev) =>
          prev.map((m) => {
            if (m.id === assistantId) {
              return { ...m, isStreaming: false, retrievalMode: mode };
            }
            const isPendingCompare =
              m.content === "Waiting to run comparison for this mode...";
            if (isPendingCompare) {
              return {
                ...m,
                content: "Comparison canceled.",
                isStreaming: false,
              };
            }
            return m;
    ...
    <WsIdCtx.Provider value={workspaceId}>
      <DebugCtx.Provider value={debugMode}>
        <AllSourcesCtx.Provider value={allSources}>
          <div className="h-full flex flex-col min-h-0 rounded-2xl bg-gradient-to-b from-card/96 via-card/88 to-card/76 overflow-hidden">
            {/* Header */}
            <div className="flex-shrink-0 flex items-center justify-between px-3.5 py-2.5 border-b border-border/70 bg-background/35">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg border border-primary/35 bg-primary/14 flex items-center justify-center">
              ...
              <div className="flex items-center gap-1.5 flex-wrap justify-end">
                <button
                  onClick={() => {
                    setQueryMode((prev) => {
                      ...
                      localStorage.setItem("nexusrag-query-mode", next);
                      localStorage.setItem(
                        "nexusrag-compare-modes",
                        String(next === "compare"),
                      );
                      localStorage.setItem(
                        "nexusrag-rag-mode",
                        next === "graphrag" ? "graphrag" : "rag",
                      );

                      toast.success(
                        next === "compare"
                          ? "Mode: Compare (RAG + GraphRAG)"
                          : next === "graphrag"
                            ? "Mode: GraphRAG"
                            : "Mode: RAG",
                      );
                  ...
                  className={cn(
                    ...
                    queryMode === "compare"
                      ? "text-orange-500 bg-orange-500/10 border-orange-500/30 hover:bg-orange-500/15"
                      : queryMode === "graphrag"
                        ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/30 hover:bg-emerald-500/15"
                        : "text-muted-foreground border-border/65 hover:bg-muted",
                  )}
                  title={
                    queryMode === "compare"
                      ? "Compare mode — run both RAG and GraphRAG"
                      : queryMode === "graphrag"
                        ? "GraphRAG mode — use knowledge graph retrieval"
                        : "RAG mode — use standard hybrid retrieval"
                  }
                >
                  <span>
                    {queryMode === "compare"
                ...
                {thinkingSupported && (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-colors border",
                      enableThinking
                        ? "text-violet-400 bg-violet-400/10 border-violet-400/30 hover:bg-violet-400/15"
                        : "text-muted-foreground border-border/65 hover:bg-muted",
                    )}
                    title={
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-colors border",
                    forceSearch
                      ? "text-amber-500 bg-amber-500/10 border-amber-500/30 hover:bg-amber-500/15"
                      : "text-muted-foreground border-border/65 hover:bg-muted",
                  )}
                  title={
                    forceSearch
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-colors border",
                    showPromptEditor
                      ? "text-blue-500 bg-blue-500/10 border-blue-500/30 hover:bg-blue-500/15"
                      : "text-muted-foreground border-border/65 hover:bg-muted",
                  )}
            ...
            <AnimatePresence>
              {showPromptEditor && (
                <motion.div
                  ...
                  className="flex-shrink-0 overflow-visible border-b border-border/70 relative z-10"
                >
                  <div className="px-3.5 py-2.5 space-y-2 bg-background/35">
                        ...
                        <div className="absolute left-0 top-full mt-1.5 z-50 w-[340px] rounded-lg border border-border bg-background shadow-xl opacity-0 pointer-events-none group-hover/cite:opacity-100 group-hover/cite:pointer-events-auto transition-opacity duration-150">
                          <div className="px-3 py-2.5">
                            <p className="text-[10px] font-semibold text-blue-700 dark:text-blue-300 mb-1.5">
                              Citation + Formatting + Restrictions (always
                              enforced)
                            </p>
            ...
            {messages.length === 0 ? (
              ...
              <div
                ...
                className="flex-1 min-h-0 overflow-y-auto px-3.5 py-3.5 space-y-3.5 relative"
              >
                <AnimatePresence>
                  {messages.map((msg) => (
            ...
            {/* Input area */}
            <div className="flex-shrink-0 p-3.5 border-t border-border/70 bg-background/30">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  rows={1}
                  className={cn(
                    "flex-1 resize-none rounded-xl border border-input bg-background/95 px-3 py-2 text-sm",
