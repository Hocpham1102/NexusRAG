...
export const DataPanel = memo(function DataPanel({
  ...
  const handleBatchProcess = useCallback(async () => {
    ...
    const processable = documents.filter((d) =>
      PROCESSABLE_STATUSES.has(d.status),
    );
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden rounded-2xl bg-gradient-to-b from-card/95 via-card/88 to-card/72">
      {/* Header — workspace name */}
      <div className="flex-shrink-0 px-3.5 pt-3.5 pb-2.5 border-b border-border/70 space-y-2 bg-background/30">
        <button
          onClick={() => navigate("/")}
      ...
      <div className="flex-shrink-0 px-3.5 py-2 flex items-center justify-between border-b border-border/70 bg-background/35">
      ...
      <div
        className="flex-shrink-0 px-3.5 pt-2.5 pb-2"
        style={{ height: "15%" }}
      >
      ...
      {/* Stats bar */}
      <div className="flex-shrink-0 px-3.5 py-2 border-b border-border/70 space-y-2 bg-background/25">
        ...
        {pendingCount > 0 && (
          <button
            ...
            className={cn(
              ...
              (batchProcessing || processingCount > 0) &&
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              <Sparkles
                className={cn(
                  "w-3.5 h-3.5 text-blue-400 flex-shrink-0",
                  batchProcessing && "animate-spin",
                )}
              />
