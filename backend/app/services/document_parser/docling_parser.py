...
class DoclingDocumentParser(BaseDocumentParser):
    ...
    def __init__(self, workspace_id: int, output_dir: Optional[Path] = None):
        ...
        self._converter_without_ocr = None
    ...
    def _build_pipeline_options(self, enable_ocr: bool):
        ...
        if enable_ocr:
            engine = settings.NEXUSRAG_OCR_ENGINE.lower().strip()
            ocr_lang = settings.NEXUSRAG_OCR_LANGUAGE.strip() or "auto"
            lang_list = [ocr_lang]

            if engine == "tesseract_cli":
                pipeline_options.ocr_options = TesseractCliOcrOptions(
                    force_full_page_ocr=settings.NEXUSRAG_OCR_FORCE_FULL_PAGE,
                    lang=lang_list,
                )
    ...
    def _get_converter(self, enable_ocr: bool = True):
        """Lazy-init Docling DocumentConverter with optional OCR."""
        if enable_ocr and self._converter is not None:
            return self._converter
        if not enable_ocr and self._converter_without_ocr is not None:
            return self._converter_without_ocr

        from docling.document_converter import DocumentConverter, PdfFormatOption

        pipeline_options = self._build_pipeline_options(enable_ocr=enable_ocr)
        ...
        if enable_ocr:
            ...
            self._converter_without_ocr = converter
    ...
    def _parse_with_docling(
        ...
        try:
            converter = self._get_converter(enable_ocr=settings.NEXUSRAG_ENABLE_OCR)
            conv_result = converter.convert(str(file_path))
        except Exception as exc:
            if settings.NEXUSRAG_ENABLE_OCR:
                logger.warning(
                    f"Docling OCR conversion failed for {file_path}: {exc}. Retrying without OCR."
                )
