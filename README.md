...
| **Formats**          | PDF, DOCX, PPTX, HTML, images                                   | PDF, DOCX, PPTX, XLSX, HTML, EPUB, images      |
...
| **OCR**              | Built-in OCR for scanned PDFs/images                            | OCR-enabled extraction for scanned inputs      |
...
- **OCR text preservation** — Scanned PDFs and images are OCR'd before chunking, so extracted text is indexed into RAG
...
| `NEXUSRAG_ENABLE_OCR`              | `true`                    | Enable OCR for scanned PDFs and image uploads                                |
| `NEXUSRAG_OCR_ENGINE`              | `auto`                    | OCR engine: `auto`, `tesseract_cli`, or `rapidocr`                           |
| `NEXUSRAG_OCR_LANGUAGE`            | `auto`                    | OCR language / language code list value                                      |
| `NEXUSRAG_OCR_FORCE_FULL_PAGE`     | `false`                   | Force full-page OCR on image-based pages; slower but sometimes more accurate |
...
_(Note: some clients still label this as an SSE connection, but use the `/mcp` endpoint to connect via Streamable HTTP)._
...
cd C:\Users\Hoc\Documents\GitHub\NexusRAG

# 1) Backend dependencies (nếu chưa có)

.\.venv\Scripts\Activate.ps1
pip install -r backend\requirements.txt

# 2) Frontend dependencies (nếu chưa có)
...
python -m uvicorn app.main:app --reload --port 8080 --host 0.0.0.0 --app-dir backend

# 5) Start frontend (terminal 2)

pnpm -C frontend dev --host 0.0.0.0 --port 5174
